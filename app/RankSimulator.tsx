...
export default function RankSimulator({ network, lang }: { network: Network; lang: Lang }) {
  ...
  return <section className="academic-simulator rank-simulator">
    ...
    {result && <div className="rank-result">
      <header><div><small>{fr ? "POSITION ANALYSÉE" : "الوضعية المحللة"}</small><h2>{result.item.city} • {result.item.track}</h2><p><span className="ltr-inline">Rang {result.rank} • Choix {result.choice}</span></p></div><span className="confidence">{result.item.status === "verified_snapshot" || result.item.status === "official_waitlist" ? (fr ? "Source forte" : "مصدر قوي") : (fr ? "Source officielle" : "مصدر رسمي")}</span></header>
