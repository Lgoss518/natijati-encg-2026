...
export default function RankSimulator({ network, lang }: { network: Network; lang: Lang }) {
  ...
  return <section className="academic-simulator rank-simulator">
    ...
    {result && <div className="rank-result">
      ...
      {nextScores && finalScores && estimate && <div className="rank-probability"><header><div><small>{fr ? "PROBABILITÉ ESTIMÉE" : "النسبة المتوقعة"}</small><strong>{network === "ena" ? nextScores[1] : finalScores[1]}%</strong><span>{network === "ena" ? (fr ? "liste d’attente 1 — 9 septembre" : "لائحة الانتظار 1 — 9 شتنبر") : (fr ? "chance finale centrale" : "الحظ النهائي المتوسط")}</span></div><i>{confidenceText(fr, estimate)} • {estimate.confidence}%</i></header><div className="rank-dual"><article><span>{network === "ena" ? (fr ? "Liste d’attente 1" : "لائحة الانتظار 1") : (fr ? "Prochaine liste" : "اللائحة الجاية")}</span><b>{nextScores[1]}%</b><small>{network === "ena" ? (fr ? "Mercredi 9 septembre 2026" : "الأربعاء 9 شتنبر 2026") : (fr ? "9 septembre ou prochaine vague" : "9 شتنبر أو الموجة القادمة")}</small></article><article className="central"><span>{network === "ena" ? (fr ? "Liste d’attente 2" : "لائحة الانتظار 2") : (fr ? "Final estimé" : "النهائي المتوقع")}</span><b>{finalScores[1]}%</b><small>{network === "ena" ? (fr ? "Lundi 14 septembre 2026" : "الإثنين 14 شتنبر 2026") : (fr ? "après désistements et améliorations" : "بعد الانسحابات وتحسين الاختيارات")}</small></article></div>{network === "ena" ? <div className="rank-scenarios"><article><span>{fr ? "Liste 1 — proche" : "لائحة 1 — الأقرب"}</span><b>{nextScores[1]}%</b><small><span className="ltr-inline">Rang ≈ {estimate.next[1]}</span></small></article><article className="central"><span>{fr ? "Liste 2 — proche" : "لائحة 2 — الأقرب"}</span><b>{finalScores[1]}%</b><small><span className="ltr-inline">Rang ≈ {estimate.final[1]}</span></small></article><article><span>{fr ? "Liste 2 — optimiste" : "لائحة 2 — متفائل"}</span><b>{finalScores[2]}%</b><small><span className="ltr-inline">Rang ≈ {estimate.final[2]}</span></small></article></div> : <div className="rank-scenarios"><article><span>{fr ? "Prudent" : "متشائم"}</span><b>{finalScores[0]}%</b><small><span className="ltr-inline">Rang ≈ {estimate.final[0]}</span></small></article><article className="central"><span>{fr ? "Central" : "متوسط"}</span><b>{finalScores[1]}%</b><small><span className="ltr-inline">Rang ≈ {estimate.final[1]}</span></small></article><article><span>{fr ? "Optimiste" : "متفائل"}</span><b>{finalScores[2]}%</b><small><span className="ltr-inline">Rang ≈ {estimate.final[2]}</span></small></article></div>}</div>}
