...
function estimateScenarios(network: Network, item: Item): Estimate {
  if (network === "health") {
    ...
    const trackBase = item.track === "Pharmacie" ? .31 : item.track === "Dentaire" ? .25 : .16;
    const movement = Math.max(.10, Math.min(.46, trackBase + (.68 - firstShare) * .19));
    const central = Math.max(5, Math.round(max * movement));
    const next = [Math.round(central * .78), central, Math.round(central * 1.5)] as [number, number, number];
    const final = next;
    const confidence = item.track === "Medecine" ? 82 : 79;
  ...
  if (network === "ena") {
    ...
    const base = rankHorizons.ena[item.city] || [35, 80, 140];
    const next = [base[0], base[1], base[2]].map((value) => Math.max(8, Math.round(value * .58))) as [number, number, number];
    const final = base;
...
export default function RankSimulator({ network, lang }: { network: Network; lang: Lang }) {
  ...
  function analyze(event: FormEvent) {
    ...
    if (mode === "massar") {
      ...
      const candidate = catalog?.candidates?.find((entry) => entry.code === code && entry.city === city && (network !== "health" || entry.track === track));
      if (!candidate) { setError(fr ? (network === "health" ? "Code Massar non trouvé dans les listes du 4 août 2026. Vérifiez les caractères ou utilisez votre rang." : "Code Massar non trouvé dans les listes ENA intégrées pour cette ville. Essayez le rang.") : (network === "health" ? "ما لقيناش Code Massar فلوائح 4 غشت 2026. راجع الحروف أو استعمل الرتبة." : "ما لقيناش Code Massar فلوائح ENA المدمجة لهاد المدينة. جرّب الرتبة.")); return; }
      const item = catalog?.items.find((entry) => entry.city === candidate.city && entry.track === candidate.track);
      if (!item) return;
      setChoice(String(candidate.choice)); setResult({ item, rank: candidate.rank, choice: candidate.choice, code });
      return;
    }
  ...
  const before = result ? (catalog?.candidates || []).filter((candidate) => candidate.city === result.item.city && candidate.track === result.item.track && candidate.rank < result.rank) : [];
  const realCompetitors = Math.round(before.reduce((total, candidate) => total + (candidate.choice === 1 ? 1 : candidate.choice === 2 ? .55 : .3), 0));
  ...
  return <section className="academic-simulator rank-simulator">
    ...
    <div className="academic-card"><form onSubmit={analyze}>
      {(network === "ena" || network === "health") && <div className="mode-tabs wide"><button type="button" className={mode==="rank"?"active":""} onClick={()=>{setMode("rank");setRank("100")}}>{fr ? "J’ai mon rang" : "عندي الرتبة"}</button><button type="button" className={mode==="massar"?"active":""} onClick={()=>{setMode("massar");setRank("")}}>{fr ? "J’ai mon Code Massar" : "عندي Code Massar"}</button></div>}
    ...
    {result && <div className="rank-result">
      ...
      {result.item.status === "verified_snapshot" ? <div className="rank-facts"><article><small>{fr ? "Candidats encore classés" : "المترشحين اللي باقين"}</small><strong>{result.item.candidates_remaining}</strong></article><article><small>{fr ? "Concurrents réels estimés avant vous" : "المنافسين الحقيقيين قبلك"}</small><strong>{realCompetitors}</strong></article><article><small>{fr ? "Position relative" : "الموقع داخل اللائحة"}</small><strong>{relative}%</strong></article></div> : null}
      {nextScores && finalScores && estimate && <div className="rank-probability"><header><div><small>{fr ? "PROBABILITÉ ESTIMÉE" : "النسبة المتوقعة"}</small><strong>{nextScores[1]}%</strong><span>{fr ? "prochaine liste — scénario le plus proche" : "اللائحة الجاية — السيناريو الأقرب"}</span></div><i>{confidenceText(fr, estimate)} • {estimate.confidence}%</i></header>{network === "health" ? <div className="rank-scenarios health-scenarios"><article className="central"><span>{fr ? "Scénario le plus proche" : "السيناريو الأقرب"}</span><b>{nextScores[1]}%</b><small><span className="ltr-inline">Rang ≈ {estimate.next[1]}</span></small></article><article><span>{fr ? "Scénario optimiste" : "سيناريو متفائل"}</span><b>{nextScores[2]}%</b><small><span className="ltr-inline">Rang ≈ {estimate.next[2]}</span></small></article></div> : <div className="rank-dual"><article><span>{fr ? "Liste d’attente 1" : "لائحة الانتظار 1"}</span><b>{nextScores[1]}%</b><small>{fr ? "Mercredi 9 septembre 2026" : "الأربعاء 9 شتنبر 2026"}</small></article><article className="central"><span>{fr ? "Liste d’attente 2" : "لائحة الانتظار 2"}</span><b>{finalScores[1]}%</b><small>{fr ? "Lundi 14 septembre 2026" : "الإثنين 14 شتنبر 2026"}</small></article></div>}</div>}
