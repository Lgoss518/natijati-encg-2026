...
export default function AdminLogin() {
  ...
  return <main className="admin-login" dir="rtl"><form onSubmit={submit}><span>ORIENTATION LGOSS</span><h1>لوحة الإدارة</h1><p>دخل كلمة السر الخاصة بصاحب الموقع.</p><input name="password" type="password" placeholder="كلمة السر" required autoFocus/>{error&&<small>{error}</small>}<button disabled={loading}>{loading?"جاري الدخول…":"الدخول"}</button><a href="/">الرجوع للموقع</a></form></main>;
}
