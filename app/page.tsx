...
export default function Home(){
  ...
  return <main dir={fr?"ltr":"rtl"} className={`site-shell ${fr?"is-fr":"is-ar"} ${!entered?"is-locked":""} ${network!=="encg"?"network-academic":""}`}>
    ...
    <div className="site-content" aria-hidden={!entered}>
      ...
      {result&&scores&&<section id="result" className="results-wrap"><div className="result-top"><div><span className="mini">{t.result}</span><h2>{schoolName(result.school)} • {t.rank} {result.rank}</h2><p>Choix {result.choice}{result.code?` • ${result.code}`:""}</p></div><div className="score-ring" style={{"--score":`${scores.central*3.6}deg`} as React.CSSProperties}><strong>{scores.central}%</strong><span>{fr?"Final estimé":"النهائي المتوقع"}</span></div></div><div className="encg-dual"><article><span>{fr?"Prochaine liste":"اللائحة الجاية"}</span><b>{scores.pessimistic}%</b><small>{fr?"Scénario prudent du 9 septembre":"سيناريو حذر ديال 9 شتنبر"}</small></article><article className="central"><span>{fr?"Final estimé":"النهائي المتوقع"}</span><b>{scores.central}%</b><small>{fr?"Après désistements et améliorations":"بعد الانسحابات وتحسين الاختيارات"}</small></article></div><div className="chance-meter"><div className="meter-labels"><span>{fr?"Faible":"ضعيفة"}</span><span>{fr?"Moyenne":"متوسطة"}</span><span>{fr?"Bonne":"جيدة"}</span><span>{fr?"Forte":"قوية"}</span></div><div className="meter-track"><i style={{insetInlineStart:`calc(${scores.central}% - 8px)`}}/></div></div><div className="scenario-grid"><article><span>{t.prudent}</span><strong>{scores.pessimistic}%</strong><small>{t.listNear} {result.school.low}</small></article><article className="main"><span>{t.probable}</span><strong>{scores.central}%</strong><small>{t.listNear} {result.school.mid}</small></article><article><span>{t.optimistic}</span><strong>{scores.optimistic}%</strong><small>{t.listNear} {result.school.high}</small></article></div><div className="insights"><article><span className="icon">◎</span><div><small>{t.real}</small><strong>{result.real}</strong><p>{t.realText.replace("{raw}",String(result.raw))}</p></div></article><article><span className="icon">1</span><div><small>{t.first}</small><strong>{result.first}</strong><p>{t.firstText}</p></div></article></div><div className="advice"><span>{t.practical}</span><p>{advice}</p><small>{t.disclaimer}</small></div><button className="reset" onClick={()=>{setResult(null);document.getElementById("simulator")?.scrollIntoView({behavior:"smooth"})}}>{t.reset} ↻</button></section>}
