...
export default function Home(){
  ...
  useEffect(()=>{
    ...
    const refresh=()=>fetch(`/api/model?t=${Date.now()}`,{cache:"no-store"}).then(async r=>r.ok?r.json():Promise.reject()).catch(()=>fetch(`/model.json?t=${Date.now()}`,{cache:"no-store"}).then(r=>r.json())).then(model=>{
      ...
      versionRef.current=model.version||"2026.08.04";setModelVersion(versionRef.current);setAnnouncement(model.announcement||"");
  ...
  return <main dir={fr?"ltr":"rtl"} className={`site-shell ${fr?"is-fr":"is-ar"} ${!entered?"is-locked":""} ${network!=="encg"?"network-academic":""}`}>
    ...
    <div className="site-content" aria-hidden={!entered}>
      ...
      <section className="next-list-bar"><div className="next-list-copy"><span>{fr?"FENÊTRE ESTIMÉE":"موعد تقريبي"}</span><strong>{fr?"Prochaine liste attendue entre le 11 et le 19 septembre 2026":"اللائحة الجاية متوقعة ما بين 11 و19 شتنبر 2026 إن شاء الله"}</strong>{announcement&&<small>{announcement}</small>}</div><button className={`notify-button ${notifications}`} onClick={enableNotifications}>{notifications==="granted"?(fr?"✓ Alertes activées":"✓ الإشعارات خدامة"):notifications==="denied"?(fr?"Alertes bloquées":"الإشعارات مرفوضة"):(fr?"Activer les alertes":"فعّل الإشعارات")}</button></section>
