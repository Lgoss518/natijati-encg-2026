...
type Lang = "ar" | "fr";
type Network = "encg" | "est" | "fst" | "ensa" | "ensam" | "health" | "ens" | "ena";
...
const networks: Array<{ key: Network; icon: string; ar: string; fr: string; arHint: string; frHint: string }> = [
  ...
  { key:"ena", icon:"⌁", ar:"ENA", fr:"ENA", arHint:"الهندسة المعمارية", frHint:"Architecture" },
...
const networkGuides: Record<Network, { arTitle:string; frTitle:string; arInput:string; frInput:string; arOutput:string; frOutput:string; arTip:string; frTip:string }> = {
  ...
  ena: { arTitle:"ENA Architecture", frTitle:"ENA Architecture", arInput:"المدينة والرتبة، أو Code Massar للمدن اللي لوائحها مقروءة.", frInput:"Ville et rang, ou Code Massar pour les villes déjà lisibles.", arOutput:"نسبة اللائحة الجاية، النهائي المتوقع، ومؤشر الثقة حسب قوة المصدر.", frOutput:"Chance pour la prochaine liste, final estimé et indice de fiabilité.", arTip:"Rabat وFès وTétouan فيهم بحث Code Massar مدمج حالياً.", frTip:"Rabat, Fès et Tétouan disposent déjà d’une recherche Code Massar intégrée." },
...
export default function Home(){
  ...
  return <main dir={fr?"ltr":"rtl"} className={`site-shell ${fr?"is-fr":"is-ar"} ${!entered?"is-locked":""} ${network!=="encg"?"network-academic":""}`}>
    ...
    <div className="site-content" aria-hidden={!entered}>
      ...
      {(network==="ensa"||network==="ensam"||network==="health"||network==="ena")&&<RankSimulator network={network} lang={activeLang}/>}
