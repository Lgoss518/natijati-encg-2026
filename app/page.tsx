...
const copy = {
  ar: {
    navSim:"المحاكي", navSchools:"المدارس", navMethod:"المنهجية", updated:"آخر تحديث: 4 غشت 2026",
    ...
    rankLabel:"الرتبة فلائحة انتظار المدينة", rankExample:"مثال: 570", codeExample:"مثال: J142107012",
  ...
  fr: {
    navSim:"Simulateur", navSchools:"Écoles", navMethod:"Méthodologie", updated:"Mise à jour : 4 août 2026",
    ...
    rankLabel:"Rang dans la liste d’attente", rankExample:"Exemple : 570", codeExample:"Exemple : J142107012",
...
export default function Home(){
  ...
  const confidence=scores?scores.central>=75?(fr?"Confiance élevée":"ثقة عالية"):scores.central>=45?(fr?"Confiance moyenne":"ثقة متوسطة"):(fr?"Confiance prudente":"ثقة حذرة"):"";
  const smartRecommendations=useMemo(()=>[...liveSchools].map(s=>({school:s,score:chance(result?.rank||compareRank,s.mid,result?.choice||compareChoice)})).sort((a,b)=>b.score-a.score).slice(0,3),[liveSchools,result,compareRank,compareChoice]);
  const shareText=result&&scores?`ORIENTATION LGOSS\n${fr?"École":"المدرسة"}: ${schoolName(result.school)}\n${fr?"Rang":"الرتبة"}: ${result.rank}\nChoix: ${result.choice}\n${fr?"Final estimé":"القبول النهائي المتوقع"}: ${scores.central}%\nhttps://orientation-lgoss.vercel.app`:"";
  ...
  return <main dir={fr?"ltr":"rtl"} className={`site-shell ${fr?"is-fr":"is-ar"} ${!entered?"is-locked":""} ${network!=="encg"?"network-academic":""}`}>
    ...
    <div className="site-content" aria-hidden={!entered}>
      ...
      {result&&scores&&<section id="result" className="results-wrap"><div className="result-top"><div><span className="mini">{t.result}</span><h2>{schoolName(result.school)} • {t.rank} <span className="ltr-inline">{result.rank}</span></h2><p><span className="ltr-inline">Choix {result.choice}{result.code?` • ${result.code}`:""}</span></p><span className="confidence-pill">{confidence}</span></div><div className="score-ring" style={{"--score":`${scores.central*3.6}deg`} as React.CSSProperties}><strong>{scores.central}%</strong><span>{fr?"Final estimé":"النهائي المتوقع"}</span></div></div><div className="encg-dual"><article><span>{fr?"Prochaine liste":"اللائحة الجاية"}</span><b>{scores.pessimistic}%</b><small>{fr?"Scénario prudent du 9 septembre":"سيناريو حذر ديال 9 شتنبر"}</small></article><article className="central"><span>{fr?"Final estimé":"النهائي المتوقع"}</span><b>{scores.central}%</b><small>{fr?"Après désistements et améliorations":"بعد الانسحابات وتحسين الاختيارات"}</small></article></div><div className="chance-meter"><div className="meter-labels"><span>{fr?"Faible":"ضعيفة"}</span><span>{fr?"Moyenne":"متوسطة"}</span><span>{fr?"Bonne":"جيدة"}</span><span>{fr?"Forte":"قوية"}</span></div><div className="meter-track"><i style={{insetInlineStart:`calc(${scores.central}% - 8px)`}}/></div></div><div className="scenario-grid"><article><span>{t.prudent}</span><strong>{scores.pessimistic}%</strong><small>{t.listNear} <span className="ltr-inline">{result.school.low}</span></small></article><article className="main"><span>{t.probable}</span><strong>{scores.central}%</strong><small>{t.listNear} <span className="ltr-inline">{result.school.mid}</span></small></article><article><span>{t.optimistic}</span><strong>{scores.optimistic}%</strong><small>{t.listNear} <span className="ltr-inline">{result.school.high}</span></small></article></div><div className="insights"><article><span className="icon">◎</span><div><small>{t.real}</small><strong>{result.real}</strong><p>{t.realText.replace("{raw}",String(result.raw))}</p></div></article><article><span className="icon">1</span><div><small>{t.first}</small><strong>{result.first}</strong><p>{t.firstText}</p></div></article></div><div className="action-checklist"><article><b>{fr?"Que faire maintenant ?":"شنو ندير دابا؟"}</b><p>{scores.central>=70?(fr?"Gardez ce choix en priorité et activez les alertes avant la prochaine liste.":"خلي هاد الاختيار مهم وفعل الإشعارات قبل اللائحة الجاية."):scores.central>=45?(fr?"Gardez ce choix, mais préparez une option de sécurité au cas où.":"خلي هاد الاختيار، ولكن وجد بديل آمن احتياطياً."):fr?"Suivez la liste jusqu’au bout et sécurisez un autre plan réaliste.":"تابع اللائحة حتى للآخر ووجد خطة أخرى واقعية."}</p></article><article><b>{fr?"Meilleures alternatives ENCG":"أحسن البدائل داخل ENCG"}</b><div>{smartRecommendations.map(item=><span key={item.school.key}>{schoolName(item.school)} <em>{item.score}%</em></span>)}</div></article></div><div className="share-panel"><span>{fr?"Partager le résultat":"شارك النتيجة"}</span><button onClick={()=>copyShare("whatsapp")}>WhatsApp</button><button onClick={()=>copyShare("copy")}>{fr?"Copier":"نسخ"}</button><button onClick={()=>copyShare("tiktok")}>TikTok caption</button></div><div className="advice"><span>{t.practical}</span><p>{advice}</p><small>{t.disclaimer}</small></div><button className="reset" onClick={()=>{setResult(null);document.getElementById("simulator")?.scrollIntoView({behavior:"smooth"})}}>{t.reset} ↻</button></section>}
